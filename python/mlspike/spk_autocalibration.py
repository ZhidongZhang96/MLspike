...
# Scale factor to match MATLAB's percent-based amplitude reporting.
...
def _detect_events(calcium, n, fit, drift, pax, par):
    ...
    while i < len(kevent):
        ...
        kept_nn[idx1] = int(round(ni * par["a"] * AMPLITUDE_SCALE))
        lastevent = tj
        i += 1
...
def _energy(tau, events, F, dt, pax):
    ...
    for i, ev in enumerate(events):
        ...
        dif.append(Fi - pred)
        amps.append(amps_i)
    error = rms(np.concatenate(dif)) * AMPLITUDE_SCALE
